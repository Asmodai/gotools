...
levels=("info" "warn" "debug" "fatal")
callers=("test/one.go" "test/two.go" "test/three.go")
msgs=("No coffee"
"Shit we're out of nuclear fuel"
"UH OH I deleted it"
"Nothing to see here move along please."
"Oh well uh did we really need that 100TiB disk?"
"Whoops... I crashed everything"
"This is a stupidly long message that we're going to really enjoy and shit because we get to test the wrapping ability of the Go cui bullshit which I wish was Borland TVision!"
"clock speed"
"solar flares"
...
metas=("type" "signal" "state" "mood" "nothing")
luls=("panic" "laugh" "cry" "blame boss" "blame coworker" "blame Russia" "blame China")
...
makeEntry() {
  ...
  caller=${callers[ $RANDOM % ${#callers[@]} ]}
  msg=${msgs[ $RANDOM % ${#msgs[@]} ]}
  meta=${metas[ $RANDOM % ${#metas[@]} ]}
  lul=${luls[ $RANDOM % ${#luls[@]} ]}
  trace=""

  if [ ${level} == "fatal" ]
  then
    trace=",\"stacktrace\":\"jebus\n\t/home/vputin/nukes.go:666\ncrap\n\t/home/vputin/threats.go:10\nruntime.main\n\t/usr/lib/go-1.16/src/runtime/proc.go:225\""
  fi

  echo "{\"level\":\"${level}\",\"ts\":$(makeTStamp).0,\"caller\":\"${caller}\",\"msg\":\"${cnt} - ${msg}\",\"${meta}\":\"${lul}\"${trace}}"
}
